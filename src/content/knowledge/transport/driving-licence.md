---
{
  "key": "transport/driving-licence",
  "titleZh": "中国驾照换证与其他通勤工具",
  "titleEn": "Driving-licence exchange and other mobility devices",
  "descriptionZh": "完整保留中国驾照换证材料与限制的 2024 记录，并标出滑板车等旧速度规则待核验。",
  "descriptionEn": "Historical licence-exchange documents and unverified mobility-device rules from the handbook.",
  "category": "transport",
  "order": 15,
  "aliases": [
    "驾照",
    "驾驶证",
    "换驾照",
    "中国驾照",
    "滑板车",
    "平衡车",
    "driving licence",
    "driving license",
    "rijbewijs",
    "scooter",
    "hoverboard",
    "segway",
    "skateboard"
  ],
  "sourceChapter": "第七章 比利时与鲁汶交通",
  "sourceSection": "机动车／驾驶证／其他通勤工具",
  "sourcePages": [
    28,
    29
  ],
  "sourceYear": 2024,
  "reviewStatus": "partially-reviewed",
  "timeSensitive": true,
  "officialSources": [
    {
      "label": "鲁汶市驾驶证官方入口",
      "url": "https://www.leuven.be/en/driving-licences"
    },
    {
      "label": "比利时交通部外国驾照",
      "url": "https://mobilit.belgium.be/nl/weg/rijden/rijbewijzen/buitenlands-rijbewijs"
    },
    {
      "label": "Local Police — Regels voor voortbewegingstoestellen",
      "url": "https://www.politie.be/5998/nl/vragen/verkeer/regels-voor-voortbewegingstoestellen"
    }
  ],
  "related": [
    "transport/bike-rental",
    "transport/bus",
    "transport/buying-bikes"
  ],
  "quickAnswer": "先向鲁汶市确认你的中国驾照能否换证及最新材料；本文保留手册材料清单，但不能据此推定换证资格或上路权限。",
  "sourceTitle": "鲁汶学联新生手册 2024",
  "lastReviewed": "2026-09-14",
  "approvedUpdateIds": [
    "NSG-008"
  ]
}
---
## 换证资格需由主管机关确认

2024 手册称中国驾驶证可以换比利时驾驶证，并强调“入境比利时后回国再考驾照”不能据此更换。手册也称驾照需处于原有效期，后续延长有效期不能兑换。

这些涉及资格判断的历史说法**未逐项核验为当前规则**，实际还可能取决于证件和个人情况。开始办理前，请从 [鲁汶市驾驶证官方入口](https://www.leuven.be/en/driving-licences) 确认要求，不要把本页当作已经获准换证或驾驶的证明。

## 2024 手册材料清单

1. 中国驾驶证原件。
2. 有资质翻译者出具的翻译件，原文要求法院盖章。
3. 比利时 ID 卡或身份证明。
4. 照片；原文称已有比利时 ID 时可用 ID 上的照片，不必另备。

手册提到照片规格文档和中比驾照类型对应资料，但 PDF 中未保留可恢复的目标链接；这里没有猜测照片尺寸或类别对应关系。材料是否适用，仍需按市政当前要求核对。

原文未展开在比利时考驾照或机动车交通规则，要求读者另查资料；本页没有补造考试流程。

## 滑板车、平衡车等

<!-- NSG-008 -->

**2026-09-14 更新：** 先区分电动、非电动与行动辅助器具；不要再使用 6／8／18 km/h 的旧分档。普通电动滑板车按现行自行车通行规则和车辆条件使用。

参考：[Local Police](https://www.politie.be/5998/nl/vragen/verkeer/regels-voor-voortbewegingstoestellen)。

使用前应向主管部门查询适用交通规则。

手册提醒这些通勤工具可能发生事故，需注意自己和其他道路使用者的安全。
