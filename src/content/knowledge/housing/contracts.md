---
{
  "key": "housing/contracts",
  "titleZh": "租房合同：签约前逐项核对",
  "titleEn": "Rental contracts: a signing checklist",
  "descriptionZh": "包含地址、房况、租期、房租、水电、押金、税、保险、转租、解约、维修、房东进入和宠物的核对表。",
  "descriptionEn": "A source-based checklist for address, condition, lease, costs, deposit, tax, insurance, subletting, termination, maintenance, access and pets.",
  "category": "housing",
  "order": 80,
  "aliases": [
    "合同",
    "租房合同",
    "租约",
    "contract",
    "rental agreement",
    "lease",
    "押金",
    "deposit",
    "水电",
    "火灾保险",
    "城市税",
    "房东",
    "pets"
  ],
  "sourceChapter": "第四章 初到鲁汶",
  "sourceSection": "4.1 合同及注意事项：合同类型及构成、注意事项",
  "sourcePages": [
    12,
    13,
    14
  ],
  "sourceYear": 2024,
  "reviewStatus": "legacy-2024",
  "timeSensitive": true,
  "officialSources": [
    {
      "label": "补充核验入口｜KU Leuven：租户、合同与费用（非手册原链）",
      "url": "https://www.kuleuven.be/english/life-at-ku-leuven/housing/info-for-tenants"
    }
  ],
  "related": [
    "housing/deposit-and-condition",
    "housing/subletting-and-termination",
    "housing/finding-housing",
    "arrival/residence-card",
    "life/utilities",
    "life/waste-recycling"
  ],
  "quickAnswer": "逐条确认合同内容和房间情况附录，把额外协商事项留成书面记录；手册提示不懂的荷兰语条款可请 Housing Service 帮助。",
  "sourceTitle": "鲁汶学联新生手册 2024"
}
---

## 先了解使用哪种合同

手册说合同因住宿类型、房源而异。私有市场常见学校提供的模板合同，手册认为其较标准、对学生较友好；其他房东会使用自行编写或选用的合同，多数中介和公司也不使用学校模板。

非模板合同不一定较差，但条款差异较大，需要仔细阅读。

## 使用签约清单

上方互动清单涵盖手册的全部合同核对项，可保存勾选并打印。

## 水、电、暖、网要问清什么

明确哪些项目包含在房租里，哪些需要自己开通服务，例如电力、网络。对于水、电、暖等，与房东或能源公司确认是按实际使用量结算、多退少补，还是固定价格、按原文所称的“无限制使用”。实际包含范围与使用条件以核对后的合同为准。

## 地址登记：保留手册的历史说明

原文写道，外国学生可将学生宿舍作为 secondary address（在鲁汶的地址）在市政厅登记并用于居留卡办理，学生宿舍合同已给予这种地址登记许可，无需另与房东协议。

**待核实：**上述是 2024 手册中的法律与行政概括；某一实际房源是否适用于本人居留登记，需结合当前市政厅要求与合同核对，不能仅凭这段历史说明认定。

## 房东进入、宠物和额外约定

手册建议事先明确房东进入房间的权利、频率与条件，并要求每次进入前提前通知。原文认为除年度安全检查和找房高峰外通常没有其他进入理由；还提醒五、六月的找房高峰可能与考试季重叠，频繁看房会造成打扰。这些描述不替代当前法律及合同。

大部分宿舍按手册描述不允许宠物；如有需要应事先协商。家人或朋友短住、墙面改动等也在其他条款中涉及。

**合同以外的协商，应取得书面协议，或保留邮件、短信等。手册建议所有额外协议在签署合同前达成。**

## 看不懂荷兰语合同时

手册说大部分第三方房东仅提供荷兰语合同，Housing Service 可以帮助翻译条款并提供建议。原文还称英语合同同样具有法律效力、可用于市政厅注册，但如进入司法程序，作为材料需由有资质的译者翻译成荷兰语。该法律效力及程序说明尚需核实。

租期、转租与解约细节，以及押金与房况交接，见下方文章。
