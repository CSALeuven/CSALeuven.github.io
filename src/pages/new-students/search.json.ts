import { getKnowledge, getKnowledgeUpdates } from '../../lib/knowledge';
import packing from '../../data/packing-2024.json';
export async function GET() {
  const entries = await getKnowledge();
  const updates = new Map((await getKnowledgeUpdates()).map(e => [e.data.key, e.body ?? '']));
  return new Response(JSON.stringify(entries.map(e => ({
    ...
    order:e.data.order, body:((e.body ?? '') + '\n' + (updates.get(e.data.key) ?? '') + (e.data.key==='before-arrival/packing' ? '\n' + packing.map(item=>`${item.name} ${item.quantity} ${item.notes}`).join('\n') : '')).replace(/<!--[\s\S]*?-->/g,'').replace(/\[([^\]]+)\]\([^)]*\)/g,'$1').replace(/[#*>`_|]/g,'').replace(/\s+/g,' '),
